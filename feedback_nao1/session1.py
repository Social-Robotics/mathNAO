...
def provide_exercise():
    ...
    provided_exercise = get_current_exercise()
...
if __name__ == "__main__":
    ...
    load_exercises()
    load_answers()
    # beh = behavior_manager.getInstalledBehaviors()
    # print "Behaviors on the robot:"
    # print("behaviors: " + str(beh))
    open_conversation(animated_tts)
    initialize_student(animated_tts, speech_recognition, memory)
    explain_session(animated_tts)
    explain_matter(animated_tts)
    ...
    while datetime.datetime.now() < end:
        ...
        rijg = mistakes_rijgen(exercise[0], exercise[2], exercise[1])
        split = mistakes_splitsen(exercise[0], exercise[2], exercise[1])
        switch = mistakes_switched(exercise[0], exercise[2], exercise[1])
        operator_fault = mistakes_operator(exercise[0], exercise[2], exercise[1])

        if int(get_current_answer()) in switch:
            print("Answer in switched options, deleted from switch error.")
        ...
        if check_answer(answer):
            ...
            if int(answer) in rijg:
                print("Fout bij het rijgen")
                animated_tts.say("Dat is helaas niet goed. Ik denk dat je een tussenstapje vergeten bent. Probeer het nog eens een keer.")
            elif int(answer) in operator_fault:
                print("Fout bij plus/min")
                explain_operator(animated_tts, exercise[1])
            elif int(answer) in split:
                print("Fout bij het splitsen")
                explain_split(tts)
            elif int(answer) in switch:
                print("Fout bij het verbeelden")
                animated_tts.say("Je hebt het bijna goed!")
                animated_tts.say("Ik denk dat je een foutje gemaakt hebt bij het verbeelden van je getallen.")
                if len(exercise[0]) > 1:
                    explain_switch(animated_tts, exercise[0], exercise[0][0], exercise[0][1])
                if len(exercise[2]) > 1:
                    animated_tts.say(", En, ")
                    explain_switch(animated_tts, exercise[2], exercise[2][0], exercise[2][1])
                animated_tts.say("Probeer het nog eens een keer.")
            elif int(answer) in operator_fault:
                explain_operator(animated_tts)
            else:
                animated_tts.say("Helaas, niet goed. Ik vond deze ook heel lastig!")
                animated_tts.say("Probeer het nog eens een keer")
            ...
            if check_answer(answer):
                ...
                animated_tts.say("Ik zal je uitleggen hoe we tot het goede antwoord komen.")
                rijgen(animated_tts, exercise[0], exercise[2], exercise[1])
    ...
    animated_tts.say("Ik zie je snel weer, dan gaan we weer verder oefenen.")
    animated_tts.say("Dankjewel voor al het harde werk. Ik zal je nu het dansje leren. Dans je met me mee?")
    ...
    animated_tts.say("Dat was leuk! Dankjewel! Nu ga ik even rusten. Tot de volgende keer.")
